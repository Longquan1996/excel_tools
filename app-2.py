...
feature_menu.add_command(label="待定", command=batch_convert_files_window)
...
tk.Button(file_frame, text="删除文件(可多选)", command=delete_file).grid(row=0, column=2, padx=5, pady=5)

tk.Label(file_frame, text="文件列表:").grid(row=1, column=0, sticky="w")
file_display = tk.Listbox(file_frame, height=8, width=70, font=("Times New Roman", 8), selectmode=tk.MULTIPLE)
...
# select_sheet.bind("<<ComboboxSelected>>", lambda event: a.config(text=f"{select_sheet.get()}:"))
