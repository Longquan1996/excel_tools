...
# GitHub 上的 version.json 文件的 Raw 链接
UPDATE_URL = "https://raw.githubusercontent.com/Longquan1996/excel_tools/main/version.json"
...
def excel_split_window():
    ...
    flag = tk.BooleanVar()
    flag.set(True)  # 默认打开
    tk.Checkbutton(split_window, text="自动识别表头行(可能出错)", variable=flag,
                command=lambda: entry.config(state="disabled" if flag.get() else "normal")).grid(row=3, column=1, padx=5, pady=5, sticky="w")
    
    tk.Label(split_window, text="手动输入表头行:").grid(row=4, column=0, padx=5, pady=5, sticky="e")
    entry = tk.Entry(split_window, validate="key", 
    ...
    tk.Label(split_window, text="请选择拆分依据:").grid(row=5, column=0, padx=5, pady=5, sticky="e")
    select_title = ttk.Combobox(split_window, values=[], state="readonly")
    select_title.grid(row=5, column=1, padx=5, pady=5, sticky="w")
    ...
    tk.Button(split_window, text="开始拆分", command=split_excel).grid(row=6, column=1, padx=5, pady=5, sticky="w")

    # 显示拆分简要信息
    tk.Label(split_window, text="拆分数量:").grid(row=7, column=0, padx=5, pady=5, sticky="e")
    tk.Entry(split_window, textvariable=catagory_count, state="readonly", width=10).grid(row=7, column=1, padx=5, pady=5, sticky="w")
    tk.Label(split_window, text="具体:").grid(row=8, column=0, padx=5, pady=5, sticky="e")
    tk.Label(split_window, textvariable=catagory_tk, relief="sunken", anchor='w', padx=5, pady=5).grid(row=8, column=1, sticky="w")
    
    mode = tk.IntVar(value=0)
    tk.Radiobutton(split_window, text="保存为多个文件", variable=mode, value=1).grid(row=9, column=0, padx=5, pady=5, sticky="w")
    tk.Radiobutton(split_window, text="保存为多个页签", variable=mode, value=2).grid(row=9, column=1, padx=5, pady=5, sticky="w")
    ...
    tk.Button(split_window, text="保存", command=save_split_file).grid(row=10, column=1, columnspan=2, pady=5, padx=5, sticky="w")
...
def update_version():
    try:
        ...
        release_notes = version_info['changelog']
        if latest_version > CURRENT_VERSION:
            answer = messagebox.askyesno("更新", f"发现新版本 {latest_version}，是否更新？\n更新内容：\n{release_notes}")
            if answer:
                # 下载新版本
                download_url = version_info['update_url']
...
# 创建主窗口
root = tk.Tk()
root.title(f"HR表格处理工具集合 v{CURRENT_VERSION}")
...
menu_bar.add_cascade(label="更新", command=update_version)
